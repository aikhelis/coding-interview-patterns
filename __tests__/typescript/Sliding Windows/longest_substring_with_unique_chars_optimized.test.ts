...
import { longestSubstringWithUniqueCharsOptimized } from '../../../coding-interview-patterns/typescript/Sliding Windows/longest_substring_with_unique_chars_optimized';
...
test.each(uniqueCharsTestCases)('longestSubstringWithUniqueCharsOptimized("$s") -> $expected', ({ s, expected }) => {
        expect(longestSubstringWithUniqueCharsOptimized(s)).toBe(expected);
});
...
test('Performance: handle large strings', () => {
        ...
        expect(longestSubstringWithUniqueCharsOptimized(largeString)).toBe(26);
});
